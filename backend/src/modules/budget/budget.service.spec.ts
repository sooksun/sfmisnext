...
describe('BudgetService', () => {
  ...
  beforeEach(async () => {
    plnRepo = { find: jest.fn(), findOne: jest.fn(), save: jest.fn(), count: jest.fn() };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(PlnBudgetCategoryDetail), useValue: plnDetailRepo },
        { provide: getRepositoryToken(TbEstimateAcadyear), useValue: estimateRepo },
        { provide: getRepositoryToken(MasterBudgetCategory), useValue: masterRepo },
  ...
  describe('loadEstimateAcadyearGroup', () => {
    it('invalid scId (0) → คืน empty structure', async () => {
      ...
      expect(result).toEqual({ data: [], totalrealbudget: 0, totalsumbudget: 0 });
    });
    ...
    it('คำนวณ totalrealbudget จาก details', async () => {
      masterRepo.find.mockResolvedValue([{ bgCateId: 1, budgetCate: 'หมวด 1' }]);
      estimateRepo.findOne.mockResolvedValue({ eaId: 1, eaBudget: 100000, eaStatus: 1 });
      plnRepo.find.mockResolvedValue([{ pbcId: 5, bgCateId: 1, scId: 1, acadYear: 1, budgetYear: '2568', del: 0 }]);
      plnDetailRepo.find.mockResolvedValue([{ pbcId: 5, budget: 30000, bgTypeId: 2, del: 0 }, { pbcId: 5, budget: 20000, bgTypeId: 2, del: 0 }]);
    ...
    it('totalsumbudget รวม ea_budget ทุก row', async () => {
      masterRepo.find.mockResolvedValue([{ bgCateId: 1, budgetCate: 'A' }, { bgCateId: 2, budgetCate: 'B' }]);
      estimateRepo.findOne.mockResolvedValue({ eaId: 1, eaBudget: 200000, eaStatus: 1 });
  ...
  describe('checkBudgetCategoryOnYear', () => {
    it('invalid sc_id → { valid: false }', async () => {
      const result = await service.checkBudgetCategoryOnYear({ sc_id: 0, sy_id: 1, budget_date: '2568', up_by: 1 });
      expect(result.valid).toBe(false);
    });

    it('ไม่มี budget_date → { valid: false }', async () => {
      const result = await service.checkBudgetCategoryOnYear({ sc_id: 1, sy_id: 1, budget_date: '', up_by: 1 });
      expect(result.valid).toBe(false);
    });
    ...
    it('มีข้อมูลอยู่แล้ว → ไม่สร้าง default categories', async () => {
      ...
      const result = await service.checkBudgetCategoryOnYear({ sc_id: 1, sy_id: 1, budget_date: '2568', up_by: 1 });
    ...
    it('count=0 → สร้าง default categories จาก master', async () => {
      ...
      await service.checkBudgetCategoryOnYear({ sc_id: 1, sy_id: 1, budget_date: '2568', up_by: 7 });
  ...
  describe('checkBudgetCategoryOnYears', () => {
    it('ไม่มี pbc_id → flag: false', async () => {
      const result = await service.checkBudgetCategoryOnYears({ pbc_id: 0, sc_id: 1, sy_id: 1, budget_date: '2568' });
      expect(result).toMatchObject({ flag: false });
    });

    it('ไม่พบ plnBudget → flag: false', async () => {
      plnRepo.findOne.mockResolvedValue(null);
      const result = await service.checkBudgetCategoryOnYears({ pbc_id: 1, sc_id: 1, sy_id: 1, budget_date: '2568' });
      expect(result).toMatchObject({ flag: false, ms: 'ไม่พบข้อมูลหมวดงบประมาณ' });
    });

    it('คำนวณ balance_budget ถูกต้อง', async () => {
      plnRepo.findOne.mockResolvedValue({ pbcId: 1 });
      plnDetailRepo.find.mockResolvedValue([{ budget: 30000 }, { budget: 20000 }]); // total = 50000
      estimateRepo.findOne.mockResolvedValue({ eaBudget: 100000 });

      const result = await service.checkBudgetCategoryOnYears({ pbc_id: 1, sc_id: 1, sy_id: 1, budget_date: '2568' });
      expect(result).toMatchObject({ budgetProject: 100000, totalBudgetGroup: 50000, balance_budget: 50000, percent: '50.00' });
    });
  });
  ...
  describe('updateRealBudget', () => {
    it('ไม่มี pbc_id → flag: false', async () => {
      const result = await service.updateRealBudget({ pbc_id: 0, sc_id: 1, sy_id: 1, real_budget: 0 });
      expect(result).toMatchObject({ flag: false, ms: 'ไม่พบข้อมูล pbc_id' });
    });

    it('real_budget < 0 → flag: false', async () => {
      const result = await service.updateRealBudget({ pbc_id: 1, sc_id: 1, sy_id: 1, real_budget: -100 });
      expect(result).toMatchObject({ flag: false });
    });

    it('ไม่พบ plnBudget → flag: false', async () => {
      plnRepo.findOne.mockResolvedValue(null);
      const result = await service.updateRealBudget({ pbc_id: 1, sc_id: 1, sy_id: 1, real_budget: 5000 });
      expect(result).toMatchObject({ flag: false, ms: 'ไม่พบข้อมูลหมวดงบประมาณ' });
    });

    it('คำนวณ percent ถูกต้อง', async () => {
      const plnBudget = { pbcId: 1, scId: 1, acadYear: 1, del: 0, budgetYear: '2568', total: 0, percents: 0 };
      ...
      await service.updateRealBudget({ pbc_id: 1, sc_id: 1, sy_id: 1, real_budget: 50000 });
  ...
  describe('addEstimateAcadyear', () => {
    const dto = { sc_id: 1, sy_id: 1, budget_year: '2568', ea_budget: 100000, ea_status: 0, up_by: 5 };
