...
const short = (b) =>
  (typeof b === 'string' ? b : JSON.stringify(b)).slice(0, 140);
const d = (date) => {
...
async function income(date, mt, amount, detail, channel = 'bank') {
  return incomeMulti(
    date,
    [{ bg_type_id: mt, prd_detail: detail, prd_budget: amount }],
    channel,
    detail,
  );
}
async function incomeMulti(date, lines, channel, label) {
  ...
  if (existing) {
    ...
    rec(
      `[รับ ${actualDate}] ${actualLabel} = ${m(total)} (${channel})`,
      true,
      'skip exists',
    );
    return;
  ...
  const r = await post('Receive/addReceive', {
    ...
    receive_date: actualDate,
    receive_form: actualLabel,
    user_receive: UP_BY,
    up_by: UP_BY,
    cf_transaction: 0,
    receiveList: lines.map((l) => ({
      bg_type_id: l.bg_type_id,
      prd_detail: tagged(l.prd_detail),
      prd_budget: l.prd_budget,
      up_by: UP_BY,
    })),
  });
  const total = lines.reduce((s, l) => s + l.prd_budget, 0);
...
async function expense(date, no_doc, mt, amount, detail, channel = 'bank') {
  ...
  if (row?.status === 202) {
    rec(
      `[จ่าย ${actualDate}] ${actualDetail} = ${m(amount)} (${channel} ${actualNoDoc})`,
      true,
      'skip exists',
    );
    return;
  ...
  if (!row) {
    const add = await post('Invoice/addInvoice', {
      ...
      detail: actualDetail,
      amount,
      certificate_payment: toc,
      date_request: actualDate,
      user_request: UP_BY,
      sy_id: SY_ID,
      year: YEAR,
      status: 200,
      type_offer_check: toc,
      up_by: UP_BY,
    });
    if (!add.ok || add.body.flag === false) {
      rec(
        `[จ่าย ${actualDate}] ${actualDetail} = ${m(amount)}`,
        false,
        'addInvoice: ' + short(add.body),
      );
      return;
    }
    row = (
  ...
  if (!row) {
    rec(`[จ่าย ${date}] ${detail}`, false, 'rw_id not found');
    return;
  }
  if (amount >= 5000) {
    await post('Check/saveCommittee', {
      rw_id: row.rw_id,
      sc_id: SC_ID,
      member1_name: 'นายตรวจรับ หนึ่ง',
...
async function loanBorrow(date, borrowerId, mt, amount, purpose, category = 3) {
  ...
  if (existing) {
    rec(
      `[ยืม ${actualDate}] borrower#${borrowerId} ${actualPurpose} = ${m(amount)}`,
      true,
      'skip exists',
    );
    return;
  ...
  const r = await post('LoanAgreement/addLoanAgreement', {
    ...
    amount,
    borrow_date: actualDate,
    loan_category: category,
    up_by: UP_BY,
  });
  const label = `[ยืม ${actualDate}] borrower#${borrowerId} ${actualPurpose} = ${m(amount)}`;
  if (!r.ok || r.body.flag === false) {
    rec(label, false, 'add: ' + short(r.body));
    return;
  }
  const loan = (
    await db(
      'SELECT la_id FROM loan_agreement WHERE sc_id=? AND sy_id=? AND borrower_id=? AND borrow_date=? AND amount=? AND status=10 AND del=0 ORDER BY la_id DESC LIMIT 1',
      [SC_ID, SY_ID, borrowerId, actualDate, amount],
    )
  )[0];
  if (!loan) {
    rec(label, false, 'ไม่พบสัญญาสถานะรอตรวจสอบ');
    return;
  }
  const verified = await post('LoanAgreement/verifyLoan', {
    la_id: loan.la_id,
    verify_by: UP_BY,
    verify_date: actualDate,
    up_by: UP_BY,
  });
  if (!verified.ok || verified.body.flag === false) {
    rec(label, false, 'verify: ' + short(verified.body));
    return;
  }
  const approved = await post('LoanAgreement/approveLoan', {
    la_id: loan.la_id,
    approve_by: UP_BY,
    approve_date: actualDate,
    up_by: UP_BY,
  });
  if (!approved.ok || approved.body.flag === false) {
    rec(label, false, 'approve: ' + short(approved.body));
    return;
  }
  const disbursed = await post('LoanAgreement/disburseLoan', {
    la_id: loan.la_id,
    receipt_date: actualDate,
    up_by: UP_BY,
  });
  rec(
    label,
    disbursed.ok && disbursed.body.flag !== false,
    short(disbursed.body),
  );
}
async function loanReturn(date, borrowerId, cash, voucher, evNo) {
  ...
  if (existingReturn) {
    rec(
      `[ส่งใช้ ${actualDate}] borrower#${borrowerId} เงินสด ${m(cash)}+ใบสำคัญ ${m(voucher)}`,
      true,
      'skip exists',
    );
    return;
  ...
  const row = (
    await db(
      'SELECT la_id, amount FROM loan_agreement WHERE sc_id=? AND sy_id=? AND borrower_id=? AND status=1 AND del=0 ORDER BY la_id ASC LIMIT 1',
      [SC_ID, SY_ID, borrowerId],
    )
  )[0];
  if (!row) {
    rec(
      `[ส่งใช้ ${actualDate}] borrower#${borrowerId}`,
      false,
      'ไม่พบสัญญายืมค้าง',
    );
    return;
  ...
  const r = await post('LoanAgreement/returnLoan', {
    la_id: row.la_id,
    returned_date: actualDate,
    return_cash: cash,
    return_voucher_amount: voucher,
    evidence_no: tagged(evNo),
    up_by: UP_BY,
  });
  rec(
    `[ส่งใช้ ${actualDate}] borrower#${borrowerId} เงินสด ${m(cash)}+ใบสำคัญ ${m(voucher)}`,
...
async function govRev(date, revenue_type, entry_type, amount, detail) {
  ...
  if (existing) {
    rec(
      `[รายได้แผ่นดิน ${actualDate}] ${entry_type === 1 ? 'รับ' : 'นำส่ง'} ${actualDetail} = ${m(amount)}`,
      true,
      'skip exists',
    );
    return;
...
async function smp(date, mt, entry_type, amount, detail) {
  ...
  if (existing) {
    rec(
      `[ฝากส่วนราชการ ${actualDate}] ${entry_type === 1 ? 'ฝาก' : 'ถอน'} ${actualDetail} = ${m(amount)}`,
      true,
      'skip exists',
    );
    return;
...
async function bankLedger(date, ba_id, entry_type, amount, detail) {
  ...
  if (existing) {
    rec(
      `[ธนาคาร ${actualDate}] ${entry_type === 1 ? 'ฝาก' : 'ถอน'} ${actualDetail} = ${m(amount)}`,
      true,
      'skip exists',
    );
    return;
...
async function phasePrereq() {
  ...
  rec(
    `ตรวจปีงบ SY=${SY_ID} budget_year=${YEAR_BE}`,
    !!year && String(year.budget_year) === YEAR_BE,
    year ? `${year.budget_date_s}..${year.budget_date_e}` : 'ไม่พบปีงบ',
  );
  ...
  if (existingOpeningLoan) {
    ...
    const moneyType = (
      await db(
        'SELECT budget_type FROM master_budget_income_type WHERE bg_type_id=106 LIMIT 1',
      )
    )[0];
    await db(
  ...
  rec(
    'ตรวจสมุดใบเสร็จ active ตรงปีงบ',
    !!book,
    book
      ? `${book.book_code} เลขถัดไป ${book.current_no}/${book.to_no}`
      : 'ไม่พบสมุด active',
  );
}
...
async function phaseTx() {
  ...
  await income(
    ...
    105,
    30000,
    'นายประชา นามศรี บริจาคเพื่อการเรียนการสอน',
    'cash',
  );
  // 5 ต.ค.
  await income(
    ...
    105,
    10000,
    'มูลนิธิพระยาธรรมรักษ์ บริจาคทุนการศึกษา',
    'cash',
  );
  await expense(
    '2012-10-05',
    ...
    101,
    13500,
    'ค่าวัสดุการศึกษา ร้านกุมภาภัณฑ์',
    'bank',
  );
  ...
  await expense(
    ...
    107,
    30000,
    'ค่าวัสดุโครงการประชาธิปไตย บ.นภิส',
    'bank',
  );
  ...
  await expense(
    ...
    108,
    10500,
    'ค่าวัสดุวงดุริยางค์ ร้านนลมิวสิค',
    'bank',
  );
  ...
  await expense(
    ...
    103,
    6625,
    'ค่าจ้างเหมาอาหารเช้า-เย็น พักนอน',
    'cash',
  );
  ...
  await incomeMulti(
    ...
    [
      {
        bg_type_id: 104,
        prd_detail: 'ค่าเครื่องแบบนักเรียน',
        prd_budget: 77400,
      },
      ...
      {
        bg_type_id: 104,
        prd_detail: 'ค่ากิจกรรมพัฒนาคุณภาพผู้เรียน',
        prd_budget: 124320,
      },
    ],
    'bank',
    'สพฐ.โอนเรียนฟรี 15 ปี',
  );
  // 6 พ.ย.
  await expense(
    '2012-11-06',
  ...
  await expense(
    ...
    106,
    13975,
    'เบิกชดเชยอาหารกลางวัน 1,2,5-7 พ.ย.',
    'cash',
  );
  // 9 พ.ย.
  await expense(
    ...
    101,
    7280,
    'ค่าวัสดุก่อนประถม ร้านกุมภาภัณฑ์',
    'bank',
  );
  // 12 พ.ย.
  await expense(
  ...
  await income(
    ...
    103,
    66250,
    'สพป.โอนค่าอาหารนักเรียนพักนอน 25 คน',
    'bank',
  );
  // 15 พ.ย.
  await income(
    ...
    102,
    43000,
    'สพฐ.โอนปัจจัยพื้นฐานนักเรียนยากจน 86 คน',
    'bank',
  );
  await expense(
    '2012-11-15',
    ...
    106,
    13975,
    'เบิกชดเชยอาหารกลางวัน 8-9,12-14 พ.ย.',
    'cash',
  );
  // 20 พ.ย.
  await expense(
  ...
  await income(
    ...
    106,
    279500,
    'รับเงินสนับสนุนอาหารกลางวัน จากเทศบาล',
    'bank',
  );
  // 22 พ.ย.
  await expense(
    ...
    104,
    83250,
    'ค่าหนังสือเรียน บ.นกน้อยการเรียน',
    'bank',
  );
  await expense(
    '2012-11-22',
    ...
    106,
    13975,
    'เบิกชดเชยอาหารกลางวัน 15-16,19-21 พ.ย.',
    'cash',
  );
  await loanBorrow(
    '2012-11-22',
    ...
    101,
    4110,
    'นายสุชาติ ยืมไปราชการประชุมครูแกนนำวิทย์',
    1,
  );
  await smp(
    '2012-11-22',
    ...
    1,
    226395,
    'นำเงินอาหารกลางวัน (เกินอำนาจเก็บ) ฝาก สพป.',
  );
  // 23 พ.ย.
  await income(
    '2012-11-23',
  ...
  await smp(
    ...
    1,
    10000,
    'นำเงินประกันสัญญา บ.โชคพัฒนา ฝาก สพป.',
  );
  await expense(
    '2012-11-23',
    'บจ.11/56',
    102,
    25000,
    'ค่าจัดซื้อเครื่องแต่งกาย 50 คน',
    'bank',
  );
  ...
  await expense(
    ...
    102,
    10000,
    'ค่าจ้างเหมาพาหนะรับส่งนักเรียน 20 คน',
    'cash',
  );
  await expense(
    '2012-11-23',
    ...
    102,
    8000,
    'ค่าอุปกรณ์การเรียนเพิ่มเติม 16 คน',
    'bank',
  );
  // 24 พ.ย.
  ...
  await expense(
    ...
    106,
    13975,
    'เบิกชดเชยอาหารกลางวัน 22-23,26-28 พ.ย.',
    'cash',
  );
  // 30 พ.ย.
  await expense(
  ...
  await expense(
    ...
    104,
    9000,
    'ค่าจ้างเหมารถไปพิพิธภัณฑ์ไดโนเสาร์',
    'cash',
  );
  // 6 ธ.ค.
  await expense(
    ...
    106,
    11180,
    'เบิกชดเชยอาหารกลางวัน 29-30 พ.ย.,3-4 ธ.ค.',
    'cash',
  );
  // 7 ธ.ค.
  await expense(
  ...
  await expense(
    ...
    105,
    27600,
    'ค่าวัสดุการศึกษา บ.นกน้อย (รายได้สถานศึกษา)',
    'bank',
  );
  ...
  await expense(
    ...
    101,
    6450,
    'ค่าอุปกรณ์วิทยาศาสตร์ บ.มานิตย์',
    'bank',
  );
  ...
  await expense(
    ...
    106,
    11180,
    'เบิกชดเชยอาหารกลางวัน 6-7,11-12 ธ.ค.',
    'cash',
  );
  // 17 ธ.ค.
  await expense(
    ...
    109,
    3850,
    'คืนเงินประกันสัญญา หจก.เพลินจิต',
    'bank',
  );
  await income(
    '2012-12-17',
    105,
    3000,
    'รับค่าบำรุงสถานที่ บ.สยามยามาฮ่า',
    'cash',
  );
  // 18 ธ.ค.
  await expense(
    ...
    104,
    8500,
    'ค่าจัดซื้อวัสดุกิจกรรมวิชาการ ร้านเอกชัย',
    'cash',
  );
  await expense(
    '2012-12-18',
    ...
    104,
    10000,
    'ค่าอาหารเช้า-เย็น ค่ายลูกเสือ-ยุวกาชาด',
    'cash',
  );
  // 20 ธ.ค.
  await expense(
    ...
    106,
    13975,
    'เบิกชดเชยอาหารกลางวัน 13-14,17-19 ธ.ค.',
    'cash',
  );
  // 21 ธ.ค.
  await expense(
    ...
    104,
    48000,
    'ค่าอุปกรณ์คอมพิวเตอร์ หจก.โจคอมพิวเตอร์',
    'bank',
  );
  await taxReceive('2012-12-21', 448.6, 'หจก.โจคอมพิวเตอร์');
  // 24 ธ.ค.
  await loanBorrow(
    '2012-12-24',
    103,
    101,
    29500,
    'นางสุดสวย ยืมไปราชการแข่งทักษะวิชาการ',
    1,
  );
  // 27 ธ.ค.
  await expense(
    ...
    106,
    13975,
    'เบิกชดเชยอาหารกลางวัน 20-21,24-26 ธ.ค.',
    'cash',
  );
  await income(
    '2012-12-27',
    105,
    15000,
    'เทศบาลสนับสนุนโครงการคุณหนูแสนดี',
    'cash',
  );
  await loanBorrow(
    '2012-12-27',
    ...
    104,
    17000,
    'นายทองดี ยืมจัดกิจกรรมเข้าค่ายคุณธรรม (เรียนฟรี-กิจกรรมพัฒนาผู้เรียน)',
    3,
  );
  // 28 ธ.ค.
  await expense(
  ...
  await expense(
    ...
    106,
    25500,
    'จัดซื้ออุปกรณ์เครื่องครัว ร้านสุภา (โครงการอาหารกลางวัน)',
    'bank',
  );
...
async function phaseVerify() {
  console.log('\n── Phase 3: ตรวจสอบผล ──');
  const ft = await db(
    'SELECT type, COUNT(*) c, SUM(amount) amt FROM financial_transactions WHERE sc_id=? AND sy_id=? AND bg_type_id BETWEEN 101 AND 120 AND del=0 GROUP BY type',
    [SC_ID, SY_ID],
  );
  console.log('financial_transactions:', JSON.stringify(ft));
  const byType = await db(
    'SELECT bg_type_id, SUM(CASE WHEN type=1 THEN amount ELSE -amount END) net, COUNT(*) c FROM financial_transactions WHERE sc_id=? AND sy_id=? AND bg_type_id BETWEEN 101 AND 120 AND del=0 GROUP BY bg_type_id ORDER BY bg_type_id',
    [SC_ID, SY_ID],
  );
  console.table(
    byType.map((r) => ({
  ...
  const loans = await db(
    'SELECT borrower_name, amount, status FROM loan_agreement WHERE sc_id=? AND sy_id=? AND del=0 ORDER BY la_id',
    [SC_ID, SY_ID],
  );
  console.log('loans:', JSON.stringify(loans));
  const gov = await db(
    'SELECT revenue_type, entry_type, SUM(amount) amt FROM gov_revenue_entry WHERE sc_id=? AND sy_id=? AND budget_year=? AND del=0 GROUP BY revenue_type, entry_type',
    [SC_ID, SY_ID, YEAR_BE],
  );
  console.log('gov_revenue:', JSON.stringify(gov));
  const smpRows = await db(
    'SELECT entry_type, SUM(amount) amt FROM smp_deposit_entry WHERE sc_id=? AND sy_id=? AND budget_year=? AND del=0 GROUP BY entry_type',
    [SC_ID, SY_ID, YEAR_BE],
  );
  ...
  for (const t of [101, 102, 103, 104, 105, 106, 107, 108, 109, 110]) {
    const r = await get(
      `Register_control_money_type/load_register_control_money_type/${t}/${SC_ID}/${SY_ID}/${YEAR}`,
    );
    const j = r.body;
...
async function main() {
  await L.login();
  console.log(
    'logged in. SC=%d SY=%d YEAR(CE)=%s YEAR(BE)=%s',
    SC_ID,
    SY_ID,
    YEAR,
    YEAR_BE,
